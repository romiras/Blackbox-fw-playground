...
**)
	...
	IMPORT SYSTEM, OSA, Dialog, Strings, Files, RW := FwReaderWriter;
...
(*	PROCEDURE^ (VAR wr: Writer) WriteSChar* (x: SHORTCHAR), NEW;
	...
	BEGIN
		h := NIL; i := 0; RW.ReadSChar(rd.rider, kind);
		WHILE kind = newExt DO
			RW.ReadXString(rd.rider, path[i]);
			AddPathComp(rd); INC(rd.nextTypeId);
			IF path[i] # elemTName THEN INC(i) END;
			RW.ReadSChar(rd.rider, kind)
		END;
	...
	BEGIN
		...
		WHILE i < n DO
			...
			IF id >= 0 THEN
				IF h # NIL THEN AddBaseId(h, extId, id) END;
				RW.WriteSChar(wr.rider, oldType); RW.WriteInt(wr.rider, id); n := i
			ELSE
				IF i + 1 < n THEN RW.WriteSChar(wr.rider, newExt) ELSE RW.WriteSChar(wr.rider, newBase) END;
				RW.WriteXString(wr.rider, path[i]);
				...
				IF path[i] = modelTName THEN
					...
					RW.WriteSChar(wr.rider, newExt); RW.WriteXString(wr.rider, elemTName);
					IF h # NIL THEN AddBaseId(h, extId, wr.nextTypeId) END;
	...
	PROCEDURE (VAR rd: Reader) ReadVersion* (min, max: INTEGER; OUT version: INTEGER), NEW;
		VAR v: BYTE;
	BEGIN
		RW.ReadByte(rd.rider, v); version := v;
	...
	BEGIN
		...
		RW.ReadSChar(rd.rider, kind);
		IF kind = nil THEN
			RW.ReadInt(rd.rider, comment); RW.ReadInt(rd.rider, next);
			rd.st.end := rd.Pos();
			IF (next > 0) OR ((next = 0) & ODD(comment)) THEN rd.st.next := rd.st.end + next ELSE rd.st.next := 0 END;
			x := NIL
		ELSIF kind = link THEN
			RW.ReadInt(rd.rider, id); RW.ReadInt(rd.rider, comment); RW.ReadInt(rd.rider, next);
			rd.st.end := rd.Pos();
			IF (next > 0) OR ((next = 0) & ODD(comment)) THEN rd.st.next := rd.st.end + next ELSE rd.st.next := 0 END;
		...
		ELSIF (kind = store) OR (kind = elem) THEN
			...
			RW.ReadInt(rd.rider, comment);
			pos1 := rd.Pos();
			RW.ReadInt(rd.rider, next); RW.ReadInt(rd.rider, down); RW.ReadInt(rd.rider, len);
			pos := rd.Pos();
	...
	BEGIN
		...
		ELSE
			...
			wr.noDomain := TRUE;
			wr.modelType := OSA.ThisType(modelTName);
	...
	PROCEDURE (VAR wr: Writer) WriteVersion* (version: INTEGER), NEW;
	BEGIN
		RW.WriteByte(wr.rider, SHORT(SHORT(version)))
	END WriteVersion;

	PROCEDURE (VAR wr: Writer) WriteStore* (x: Store), NEW;
		VAR t: OSA.Type; pos1, pos2, pos: INTEGER;
			save: WriterState;
	BEGIN
		ASSERT(wr.rider # NIL, 20);
		IF x # NIL THEN
		...
		IF wr.st.linkpos > 0 THEN	(* link to previous block's <next> or up block's <down> *)
			...
			IF pos - wr.st.linkpos = 4 THEN
				...
				*)
				wr.SetPos(wr.st.linkpos - 4); RW.WriteInt(wr.rider, 1); RW.WriteInt(wr.rider, pos - wr.st.linkpos - 4)
			ELSE
				wr.SetPos(wr.st.linkpos); RW.WriteInt(wr.rider, pos - wr.st.linkpos - 4)
			END;
			wr.SetPos(pos)
		END;
		IF x = NIL THEN
			RW.WriteSChar(wr.rider, nil);
			RW.WriteInt(wr.rider, 0);	(* <comment> *)
			wr.st.linkpos := wr.Pos();
			RW.WriteInt(wr.rider, 0)	(* <next> *)
		ELSIF x.era >= wr.era THEN
			ASSERT(x.era = wr.era, 23);
			IF x.isElem THEN RW.WriteSChar(wr.rider, link) ELSE RW.WriteSChar(wr.rider, newlink) END;
			RW.WriteInt(wr.rider, x.id);
			RW.WriteInt(wr.rider, 0);	(* <comment> *)
			wr.st.linkpos := wr.Pos();
			RW.WriteInt(wr.rider, 0)	(* <next> *)
		ELSE
			...
			WITH x: Alien DO
				IF x.isElem THEN
					RW.WriteSChar(wr.rider, elem); x.id := wr.nextElemId; INC(wr.nextElemId)
				ELSE
					RW.WriteSChar(wr.rider, store); x.id := wr.nextStoreId; INC(wr.nextStoreId)
				END;
				WritePath(wr, x.path)
			ELSE
				t := OSA.TypeOf(x);
			...
			END;
			RW.WriteInt(wr.rider, 0);	(* <comment> *)
			pos1 := wr.Pos(); RW.WriteInt(wr.rider, 0); RW.WriteInt(wr.rider, 0);	(* <next>, <down> *)
			pos2 := wr.Pos(); RW.WriteInt(wr.rider, 0);	(* <len> *)
			...
			wr.st.linkpos := pos1;
			pos := wr.Pos();
			wr.SetPos(pos2); RW.WriteInt(wr.rider, pos - pos2 - 4);	(* patch <len> *)
			wr.SetPos(pos)
		END;
		wr.writtenStore := x
	END WriteStore;
